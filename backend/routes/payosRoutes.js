  ...
  try {
    ...
    if (!payosClient) {
      return res.status(503).json({
        error:
          "PayOS chưa được cấu hình. Vui lòng cấu hình PAYOS_CLIENT_ID, PAYOS_API_KEY, và PAYOS_CHECKSUM_KEY trong file .env",
      });
    }
    ...
    const paymentLink = await payosClient.paymentRequests.create({
      ...
      amount: amountNumber,
      description: `#${orderId}`,
      returnUrl: `http://localhost:3000/success?orderId=${orderId}&sessionId=${sessionId}`,
    ...
    if (!paymentLink?.checkoutUrl) {
      return res
        .status(500)
        .json({ error: "Không lấy được checkoutUrl từ PayOS" });
    }
