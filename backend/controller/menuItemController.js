  ...
  try {
    ...
    if (req.query.needPreOrder !== undefined) {
      ...
      if (needPreOrder === undefined) {
        return res
          .status(400)
          .json({ error: "needPreOrder must be true or false" });
      }
...
exports.createMenuItem = async (req, res) => {
  try {
    const { name, description, price, isAvailable, needPreOrder, category } =
      req.body;
    ...
    if (
      needPreOrder !== undefined &&
      typeof needPreOrder !== "boolean" &&
      needPreOrder !== "true" &&
      needPreOrder !== "false"
    ) {
    ...
    if (req.file) {
      ...
      const isAllowedType =
        !!req.file.mimetype && allowedTypes.includes(req.file.mimetype);
      ...
      if (!isAllowedType) {
        ...
        console.warn(
          "Unsupported image type, skipping upload:",
          req.file.mimetype,
        );
      } else if (isTooLarge) {
        ...
        try {
          ...
          const ext =
            (req.file.mimetype && req.file.mimetype.split("/")[1]) || "png";
          ...
          console.error(
            "Failed to save image locally, continuing without image:",
            localErr,
          );
        }
      } else {
...
exports.updateMenuItem = async (req, res) => {
  try {
    ...
    if (needPreOrder !== undefined)
      updateData.needPreOrder =
        needPreOrder === "true" || needPreOrder === true;
