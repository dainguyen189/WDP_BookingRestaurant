  ...
  const mergeOrdersPreservingEdits = (newOrders, prevOrders) => {
    return newOrders.map(newOrder => {
      ...
      const mergedItems = newOrder.items.map(newItem => {
        ...
        // Preserve updatedQuantity và notes từ prevItem nếu có
        const merged = { ...newItem };
        if (prevItem.updatedQuantity !== undefined) {
          merged.updatedQuantity = prevItem.updatedQuantity;
        }
        // Ưu tiên notes từ newItem (API), nếu không có thì dùng từ prevItem
        if (!newItem.notes && prevItem.notes) {
          merged.notes = prevItem.notes;
        }
