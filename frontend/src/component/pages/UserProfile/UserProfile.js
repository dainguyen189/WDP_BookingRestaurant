...
import { faCalendarAlt, faClock, faUser, faUtensils, faPhone, faEnvelope, faCheckCircle, faTimesCircle, faHourglassHalf } from '@fortawesome/free-solid-svg-icons';
...
const UserProfile = () => {
    ...
    const handleCancelReservation = async (reservationId, reservationName) => {
        ...
        if (!window.confirm(`Bạn có chắc chắn muốn hủy đặt bàn "${reservationName}" không?`)) {
            return;
        }
        ...
        try {
            ...
            showMessage('error', error.response?.data?.error || error.response?.data?.message || 'Không thể hủy đặt bàn');
        }
    };
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            ...
            case 'pending':
                return { icon: faHourglassHalf, color: '#ffc107', text: 'Chờ xác nhận' };
            case 'cancelled':
    ...
    return (
        <div className="profile-container">
            ...
            <div className="profile-content">
                <div className={`profile-card ${activeTab === 'reservations' ? 'profile-card-wide' : ''}`}>
                                            ...
                                            {reservations.map((reservation) => {
                                                ...
                                                return (
                                                    <tr key={reservation._id} className="table-row">
                                                        ...
                                                        <td>{formatDateShort(reservation.reservationDate)}</td>
                                                        ...
                                                        <td>
                                                            {canCancelReservation(reservation) ? (
                                                                <button
                                                                    ...
                                                                    onClick={() => handleCancelReservation(reservation._id, reservation.name)}
                                                                    style={{
                                                                        padding: '5px 12px',
                                                                        fontSize: '0.85rem',
                                                                        borderRadius: '4px',
