...
function QRCodeComponent({ sessionId }) {
  ...
  const handlePrint = async () => {
    ...
    doc.text(`Session ID: ${sessionId}`, pageWidth / 2, 60 + imgHeight + 30, {
      align: "center",
    });
  ...
  return (
    <div className="qr-popup-content">
      <div ref={qrRef}>
        ...
        <QRCodeCanvas
          ...
          onClick={() =>
            window.open(
              url,
              `http://localhost:3000/menu?sessionId=${sessionId}`,
            )
          }
        />
      ...
      <button onClick={handlePrint} className="qr-print-button">
        🖨️ Print QR{" "}
      </button>
    </div>
  );
